...
def similarity(query, title, category, threshold):
    query_dict = []

    print(len(title))
    count = 0
    for i, document in enumerate(title):
        count = count + 1
        score = jaro(document, query)

        if score >= threshold:
            sim_data = data_object.similarity_data(category[i], document, score)
...
set = similarity("인천공항으로 가려면", Z, K, 0.4)
for v in set:
    print(v.id,v.sentence,v.score)
